...
-- Variáveis globais para controlar se a janela foi criada


-- Função para criar o botão "Obter item pelo ID"
createButton(utilitiesWindow, "Obter item pelo ID", function()
    -- Se a janela já existe, não cria uma nova
    if itemWindowExists then
        return
    end

    -- Criar a janela para inserir o ID
    local itemWindow = createWindow("Obter Item Pelo ID", 400, 200)

    -- Criar campo de entrada para o ID do item (este campo é local à função)
    local idInputField = createTextField(itemWindow, "ID do Item:", 50, 50, 300, 30)
    
    -- Criar botão OK
    createButton(itemWindow, "OK", function()
        local itemID = idInputField:getText()  -- Pega o ID inserido no campo de texto (variável local)

        -- Verificar se o ID não está vazio e tentar obter o item
        if itemID ~= "" then
            -- Chamar a função para pegar o item pelo ID
            obtainItemByID(itemID)
        else
            showNotification("Por favor, insira um ID válido.")
        end
    end)
    
    -- Criar botão Fechar
    createButton(itemWindow, "Fechar", function()
        -- Fechar a janela
        itemWindow:close()
        itemWindowExists = false  -- Permitir criar a janela novamente (variável global)
    end)

    -- Marcar que a janela foi criada (variável global)
    itemWindowExists = true
end)

-- Função para obter o item pelo ID no Roblox
function obtainItemByID(itemID)
    local player = game.Players.LocalPlayer  -- Acessa o jogador local (variável local)
    local inventory = player.Backpack  -- Acessando o inventário do jogador (variável local)

    -- Verificar se o item existe no inventário
    local item = inventory:FindFirstChild(itemID)  -- Tenta encontrar o item no inventário (variável local)

    if item then
        -- Verificar se o item é uma ferramenta (tool)
        if item:IsA("Tool") then
            -- Se for uma ferramenta, equipar automaticamente
            showNotification("Você obteve e equipou o item: " .. item.Name)  -- Notifica que o item foi equipado
            -- O item será automaticamente equipado ao ser adicionado no "Backpack" (não é necessário código adicional aqui)
        else
            -- Caso o item não seja usável (não é uma ferramenta)
            showNotification("O item encontrado não é usável: " .. item.Name)
        end
    else
        -- Caso o item não seja encontrado
        showNotification("Item não encontrado com ID: " .. itemID)
    end
end

-- Função para exibir notificações (ajuste conforme seu sistema)
function showNotification(message)
    print(message)  -- Para testes, imprime no console. No jogo, use uma função de UI.
end
end)
